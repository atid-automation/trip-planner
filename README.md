...
- **Version 2** added **user accounts, secure registration/login, JWT-based authentication, and per-user trip ownership scoping**.
- **Version 3** added **trip-level budget management with currency support, full expense CRUD ledger, and automated over-budget detection**.
- **Version 4** added a **full Travel Journal CRUD system** with dated diary entries, plain-text multiline content, and ownership scoping.
- **Version 5** refined user experience and added a **responsive mobile interface**.
- **Version 6** migrates the persistence layer from **JSON files to a relational SQLite database (`data/app.db`)**. Relational tables with foreign keys and cascade deletions are introduced while preserving 100% of existing REST API endpoints, business logic, authorization rules, and frontend behavior.

This is intentionally small, simple, and deterministic, designed for easy testing, automated verification, and future extension.

## What's New in Version 6

- **SQLite Database Persistence**: Replaced file-based JSON storage (`trips.json`, `users.json`) with an ACID-compliant relational SQLite database located at `data/app.db`.
- **Relational Schema**: Six structured tables representing core domain entities:
  - `users`: User profiles with unique email constraints and password hashes
  - `trips`: Trips scoped to owning users via foreign keys
  - `trip_days`: Itinerary days referencing trips
  - `activities`: Scheduled attractions and activities referencing trip days
  - `expenses`: Cost entries with category, amount, currency, and date
  - `journal_entries`: Travel journal notes with title, content, date, and timestamps
- **Foreign Key Enforcement & Cascade Deletions**: SQLite foreign keys are enforced on every database connection (`PRAGMA foreign_keys = ON;`). Deleting a trip automatically cascades and removes its itinerary days, activities, expenses, and journal entries.
- **Dedicated Persistence Layer**: Clean architectural separation maintained between API routers, business logic services (`trip_service`, `auth_service`), and persistence repositories (`trip_store`, `user_store`) backed by SQLite.
- **JSON to SQLite Migration Tool**: Provided `python migrate_to_sqlite.py` to seamlessly read existing Version 5 JSON records and populate the SQLite database while preserving all IDs and relationships.
- **Database Initialization & Reset Tooling**: Provided `python init_db.py` to initialize schemas and seed deterministic dev data (`--reset` to restore a clean state anytime).
- **API & UI Compatibility**: Zero changes required on the frontend or existing test automation suites. All REST URLs, payloads, `data-testid` attributes, and error responses remain identical.
- **QA & Automation Inspection**: Clean, human-readable table and column names (`users.id`, `trips.user_id`, `expenses.trip_id`, etc.) enabling students to run direct SQL inspection queries.
...
### Database
- **SQLite 3** — Relational database embedded via Python standard library `sqlite3`
- **Location**: `data/app.db`
- **Foreign Key Enforcement**: Enabled on connection (`PRAGMA foreign_keys = ON;`)
- **Cascades**: `ON DELETE CASCADE` across all parent-child entity relationships
...
### Data Persistence
- **SQLite Database**: `data/app.db` (Version 6 primary persistence)
- **Legacy JSON Files**: `data/trips.json`, `data/users.json` (Version 5 archive, used by `migrate_to_sqlite.py`)
...
- In SQLite, deleting a trip via `DELETE /api/trips/{trip_id}` triggers foreign key cascade (`ON DELETE CASCADE`) which automatically and atomically deletes all its trip days, activities, expenses, and journal entries with zero orphaned records
...
## Database Architecture & SQLite Persistence (Version 6)

Version 6 migrates the persistence layer from JSON files (`data/trips.json`, `data/users.json`) to a local SQLite database (`data/app.db`).

### SQLite Database Location

The database is stored in the project's `data/` directory:

```text
data/
    app.db       # Primary SQLite database file
    trips.json   # Legacy Version 5 archive
    users.json   # Legacy Version 5 archive
```

You can optionally override the location by setting the environment variable `TRIP_PLANNER_DB=/path/to/database.db`.
...
erDiagram
    ...
    trips ||--o{ trip_days : "contains (1:N)"
    trips ||--o{ expenses : "contains (1:N)"
    trips ||--o{ journal_entries : "contains (1:N)"
    trip_days ||--o{ activities : "contains (1:N)"
...
1. **`users`**:
   - `id` (TEXT, PRIMARY KEY): Unique identifier (e.g. `user-alice`, `user-bob`).
   - `full_name` (TEXT, NOT NULL): User's display name.
   - `email` (TEXT, NOT NULL, UNIQUE): Unique email address (case-insensitive checks at application level).
   - `password_hash` (TEXT, NOT NULL): Adaptive bcrypt hash of password.

2. **`trips`**:
   - `id` (TEXT, PRIMARY KEY): Unique trip identifier (e.g. `trip-1`).
   ...
   - `budget` (REAL, NOT NULL, DEFAULT 0.0): Numeric budget amount.
   - `currency` (TEXT, NOT NULL, DEFAULT 'USD'): ISO 3-letter currency code.

3. **`trip_days`**:
   - `id` (TEXT, PRIMARY KEY): Unique day identifier (e.g. `day-1-1`).
   - `trip_id` (TEXT, NOT NULL, REFERENCES `trips(id)` ON DELETE CASCADE): Parent trip.
   - `date` (TEXT, NOT NULL): ISO date (`YYYY-MM-DD`).
   - `title` (TEXT): Optional day heading.

4. **`activities`**:
   - `id` (TEXT, PRIMARY KEY): Unique activity identifier (e.g. `act-1-1-1`).
   - `day_id` (TEXT, NOT NULL, REFERENCES `trip_days(id)` ON DELETE CASCADE): Parent trip day.
   - `name` (TEXT, NOT NULL): Activity title.
   - `location` (TEXT): Optional location.
   - `description` (TEXT): Optional notes.

5. **`expenses`**:
   - `id` (TEXT, PRIMARY KEY): Unique expense identifier (e.g. `exp-1-1`).
   - `trip_id` (TEXT, NOT NULL, REFERENCES `trips(id)` ON DELETE CASCADE): Parent trip.
   - `description` (TEXT, NOT NULL): Expense description.
   - `category` (TEXT, NOT NULL): Accommodation, Food, Transportation, Activities, Shopping, or Other.
   - `amount` (REAL, NOT NULL): Decimal amount (must be > 0).
   - `currency` (TEXT, NOT NULL): Matches trip currency.
   - `date` (TEXT, NOT NULL): ISO date within trip date range.

6. **`journal_entries`**:
   - `id` (TEXT, PRIMARY KEY): Unique journal entry identifier (e.g. `je-1-1`).
   - `trip_id` (TEXT, NOT NULL, REFERENCES `trips(id)` ON DELETE CASCADE): Parent trip.
   - `title` (TEXT, NOT NULL): Entry title (1–100 chars).
   - `content` (TEXT, NOT NULL): Plain text content (1–5000 chars).
   - `date` (TEXT, NOT NULL): ISO date within trip date range.
   - `created_at` (TEXT, NOT NULL): ISO 8601 UTC timestamp.
   - `updated_at` (TEXT, NOT NULL): ISO 8601 UTC timestamp.

### Foreign Key Integrity & Cascades

SQLite foreign keys are enabled on every connection via:
```sql
PRAGMA foreign_keys = ON;
```
All child tables define foreign keys with `ON DELETE CASCADE`:
- Deleting a `user` automatically cascades and deletes all their `trips` (and child records).
- Deleting a `trip` automatically cascades and deletes all its `trip_days`, `activities`, `expenses`, and `journal_entries`.
- Deleting a `trip_day` automatically cascades and deletes its `activities`.

### Database Initialization & Seed Data

The database can be initialized or reset with deterministic seed data using:

```bash
# Initialize schema and seed data (if database is empty):
python init_db.py

# Drop existing tables, re-create schema, and re-populate deterministic seed data:
python init_db.py --reset
```

The seed dataset contains:
- **Users**: Alice Smith (`alice@example.com`) and Bob Johnson (`bob@example.com`), password: `Password123!`
- **Trips**:
  - Alice owns `trip-1` (Vietnam Adventure) and `trip-3` (European Backpacking).
  - Bob owns `trip-2` (Tokyo Discovery).
- **Itinerary Days & Activities**: 32 days with 186 activities.
- **Expenses**: 15 categorized expenses across currencies USD, JPY, EUR.
- **Journal Entries**: 6 dated journal entries demonstrating newest-first ordering.

### Migrating Version 5 JSON Data to SQLite

To migrate existing JSON data from `data/users.json` and `data/trips.json` into `data/app.db`:
...
1. Creates the schema and tables in `data/app.db` if not present.
2. Migrates all users preserving existing passwords and user IDs.
3. Migrates all trips, itinerary days, activities, expenses, and journal entries.
4. Preserves all relational identifiers and foreign key associations.
5. Employs conflict resolution (`ON CONFLICT DO UPDATE`) ensuring safe, repeatable runs.

### Resetting the Database

To return the application to a clean, deterministic state during automated testing:
...
```json
{
  "message": "Data reset to seed state successfully"
}
```

### Inspecting the Database (For QA & Automation Engineers)

You can inspect the SQLite database directly using the `sqlite3` CLI or any GUI viewer (such as [DB Browser for SQLite](https://sqlitebrowser.org/)):

```bash
sqlite3 data/app.db
```

#### Example QA SQL Queries
...
**2. List all trips with budget information:**
```sql
SELECT id, user_id, name, destination, budget, currency, start_date, end_date FROM trips;
```

**3. Query trips for a specific user (Alice):**
```sql
SELECT id, name, destination, start_date, end_date, budget, currency
FROM trips
WHERE user_id = 'user-alice';
```

**4. Inspect all expenses for a specific trip:**
```sql
...
**5. JOIN Query: User names with their trips and total spent:**
```sql
SELECT 
    ...
    (t.budget - COALESCE(SUM(e.amount), 0.0)) AS remaining_budget
FROM users u
JOIN trips t ON t.user_id = u.id
LEFT JOIN expenses e ON e.trip_id = t.id
GROUP BY t.id
ORDER BY u.full_name, t.start_date;
```

**6. JOIN Query: Itinerary activities with their day date and title:**
```sql
SELECT 
    t.name AS trip_name,
    td.date AS day_date,
...
JOIN activities a ON a.day_id = td.id
WHERE t.id = 'trip-1'
ORDER BY td.date, a.rowid;
```

**7. Verify foreign key cascade:**
```sql
-- Count activities before deleting a trip:
SELECT COUNT(*) FROM activities WHERE day_id IN (SELECT id FROM trip_days WHERE trip_id = 'trip-1');

-- Delete trip-1:
DELETE FROM trips WHERE id = 'trip-1';

-- Confirm that all child records were automatically cascaded and removed:
SELECT COUNT(*) FROM trip_days WHERE trip_id = 'trip-1';       -- Expected: 0
SELECT COUNT(*) FROM expenses WHERE trip_id = 'trip-1';        -- Expected: 0
SELECT COUNT(*) FROM journal_entries WHERE trip_id = 'trip-1'; -- Expected: 0
```
...
- **`data-testid` attributes** on every key interactive element:
  ...
  - Journal section: `journal-section`, `add-journal-entry`, `journal-entry-card`, `journal-title`, `journal-date`, `journal-content-preview`, `edit-journal-entry`, `delete-journal-entry`, `save-journal-entry`, `journal-view-edit-btn`
  - All V1 trip/day/activity testids preserved unchanged
- **Predictable REST API with consistent JSON responses**
- **Deterministic reset endpoint**: Authenticated POST to `/api/system/reset` restores exact seed state
- **IDs use a readable prefix pattern** (`trip-*`, `day-*`, `act-*`, `exp-*`, `user-*`, `je-*`)
- **Automated Verification Suite**: Included `test_v6_backend.py` and `test_v6_api.py` for automated testing.

## Future Versions

This application has evolved across iterations:
...
- ✅ **Mobile/responsive support** — delivered in Version 5
- ✅ **SQLite database persistence** — delivered in Version 6
