...
def _validate_journal_date(entry_date: date, trip_id: str, user: User) -> None:
    date_range = trip_store.get_trip_date_range(trip_id, user.id)
    if date_range is None:
        raise TripNotFoundError(f"Trip {trip_id} not found")
    start_str, end_str = date_range
    trip_start = date.fromisoformat(start_str)
    trip_end = date.fromisoformat(end_str)
    if entry_date < trip_start or entry_date > trip_end:
        raise JournalValidationError(
            f"Journal entry date must be within trip dates ({trip_start.isoformat()} to {trip_end.isoformat()})"
        )


def list_journal_entries(trip_id: str, user: User) -> List[JournalEntry]:
    _get_trip_or_error(trip_id, user)
    entries = trip_store.get_all_journal_entries(trip_id, user.id)
    if entries is None:
        raise TripNotFoundError(f"Trip {trip_id} not found")
    return entries


def get_journal_entry(trip_id: str, entry_id: str, user: User) -> JournalEntry:
    _get_trip_or_error(trip_id, user)
    entry = trip_store.get_journal_entry_by_id(trip_id, entry_id, user.id)
    if entry is None:
        raise JournalEntryNotFoundError(f"Journal entry {entry_id} not found in trip {trip_id}")
    return entry


def create_new_journal_entry(trip_id: str, entry_create: JournalEntryCreate, user: User) -> JournalEntry:
    trip = _get_trip_or_error(trip_id, user)
    if not entry_create.title or not entry_create.title.strip():
        raise JournalValidationError("Title is required")
    if len(entry_create.title) > 100:
        raise JournalValidationError("Title must not exceed 100 characters")
    if not entry_create.content or not entry_create.content.strip():
        raise JournalValidationError("Content is required")
    if len(entry_create.content) > 5000:
        raise JournalValidationError("Content must not exceed 5000 characters")
    _validate_journal_date(entry_create.date, trip_id, user)
    entry = trip_store.create_journal_entry(trip_id, entry_create, user.id)
    if entry is None:
        raise TripNotFoundError(f"Trip {trip_id} not found")
    return entry


def update_existing_journal_entry(trip_id: str, entry_id: str, entry_update: JournalEntryUpdate, user: User) -> JournalEntry:
    _get_trip_or_error(trip_id, user)
    existing = trip_store.get_journal_entry_by_id(trip_id, entry_id, user.id)
    if existing is None:
        raise JournalEntryNotFoundError(f"Journal entry {entry_id} not found in trip {trip_id}")

    title_to_check = entry_update.title if entry_update.title is not None else existing.title
    content_to_check = entry_update.content if entry_update.content is not None else existing.content
    date_to_check = entry_update.date if entry_update.date is not None else existing.date

    if not title_to_check or not title_to_check.strip():
        raise JournalValidationError("Title is required")
    if len(title_to_check) > 100:
        raise JournalValidationError("Title must not exceed 100 characters")
    if not content_to_check or not content_to_check.strip():
        raise JournalValidationError("Content is required")
    if len(content_to_check) > 5000:
        raise JournalValidationError("Content must not exceed 5000 characters")
    if entry_update.date is not None:
        _validate_journal_date(date_to_check, trip_id, user)

    entry = trip_store.update_journal_entry(trip_id, entry_id, entry_update, user.id)
    if entry is None:
        raise JournalEntryNotFoundError(f"Journal entry {entry_id} not found in trip {trip_id}")
    return entry


def remove_journal_entry(trip_id: str, entry_id: str, user: User) -> None:
    _get_trip_or_error(trip_id, user)
    deleted = trip_store.delete_journal_entry(trip_id, entry_id, user.id)
    if not deleted:
        raise JournalEntryNotFoundError(f"Journal entry {entry_id} not found in trip {trip_id}")
